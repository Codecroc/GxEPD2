...
class GxEPD2_ist7136_COG : public GxEPD2_EPD
{
  public:
    ...
    void writeScreenBuffer(uint8_t reg, uint8_t value = 0xFF); // init controller memory (default white)
  ...
  private:
    ...
    void _Init_Full();
    void _reset();
    void _soft_start_DCDC();
    void _write_command_data(uint8_t command, uint8_t data);
    void _write_command_data(uint8_t command, uint8_t *data, size_t size);
  private:
